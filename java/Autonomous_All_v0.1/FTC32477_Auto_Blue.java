 ...
 * 
 * 起始位置：
 * - 车辆起始位置在蓝方起始区域（场地一端，与红方相对）
 * - 车辆朝向：根据实际场地调整（通常朝向场地中央或目标区域）
 * - 所有路径的第一步都是"前进24英寸"，从起始位置向前移动
 * 
 * 注意：
 * - 后两个轮子物理安装方向不同，在运动学公式中直接补偿
 * - 蓝方路径与红方镜像，转向方向相反
 * 
 * @author FTC32477
 * @version 1.0 - 蓝方自动阶段控制
 */
@Autonomous(name = "FTC32477 Auto Blue", group = "Autonomous")
public class FTC32477_Auto_Blue extends LinearOpMode {
    ...
    /**
     * 执行自动路径
     * 根据摄像头识别结果执行不同的路径（蓝方，路径与红方镜像）
     * 
     * 注意：当前摄像头功能已注释，默认执行中央路径
     * 如需使用摄像头识别，请取消注释相关代码
     */
    private void runAutonomousPath() {
        telemetry.addData("状态", "开始执行自动路径（蓝方）");
        telemetry.update();
        ...
        // 当前默认执行中央路径（摄像头功能未启用时）
        // 如需手动选择路径，可以取消注释下面的代码并注释掉executeCenterPath()
        // executeLeftPath();   // 执行左侧路径
        executeCenterPath();   // 执行中央路径（默认）
        // executeRightPath();  // 执行右侧路径
     ...
     * 1. 从起始位置前进24英寸
     * 2. 拾取物品
     * 3. 右转到左侧目标位置（蓝方镜像：红方左转对应蓝方右转）
     * 4. 前进到目标位置
     * 5. 装填并发射
     */
    private void executeLeftPath() {
        telemetry.addData("路径", "执行左侧路径（蓝方）");
        telemetry.update();
        ...
        // 步骤3：转向到左侧目标（蓝方镜像：红方左转-45度对应蓝方右转45度）
        encoderTurn(0.4, 45, 5.0); // 右转45度（红方是左转-45度）
    ...
    private void executeCenterPath() {
        telemetry.addData("路径", "执行中央路径（蓝方）");
        telemetry.update();
     ...
     * 1. 从起始位置前进24英寸
     * 2. 拾取物品
     * 3. 左转到右侧目标位置（蓝方镜像：红方右转对应蓝方左转）
     * 4. 前进到目标位置
     * 5. 装填并发射
     */
    private void executeRightPath() {
        telemetry.addData("路径", "执行右侧路径（蓝方）");
        telemetry.update();
        ...
        // 步骤3：转向到右侧目标（蓝方镜像：红方右转45度对应蓝方左转-45度）
        encoderTurn(0.4, -45, 5.0); // 左转45度（红方是右转45度）
    ...
    private void encoderTurn(double speed, double degrees, double timeoutS) {
        // 计算转向所需的轮子移动距离
        // 假设轮距为15.1英寸（需要根据实际机器人调整）
        double wheelBase = 15.1; // 英寸
